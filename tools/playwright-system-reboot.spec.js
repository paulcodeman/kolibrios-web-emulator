...
test("system reboot syscall restarts the loaded app and keeps fullscreen", async ({ page }) => {
  ...
  await page.addInitScript(() => {
    window.addEventListener("DOMContentLoaded", () => {
      const proto = window.KosEmu && window.KosEmu.ui && window.KosEmu.ui.App
        ...
        : null;
      if (!proto || proto.__rebootCapturePatched) {
        return;
      }
      ...
      proto.__rebootCapturePatched = true;
  ...
  await page.evaluate(() => {
    ...
    if (!process) {
      throw new Error("Missing active process before reboot.");
    }
    window.__rebootProcessRef = process;
  });
  ...
  await page.evaluate(() => {
    ...
    if (!process || !process.emulator) {
      throw new Error("Missing active process for system reboot syscall.");
    }
    process.emulator.writeReg(REG_EAX, 18);
    process.emulator.writeReg(REG_EBX, 9);
    process.emulator.writeReg(REG_ECX, 3);
  ...
  await page.waitForFunction(() => {
    ...
    const process = session.processes[0];
