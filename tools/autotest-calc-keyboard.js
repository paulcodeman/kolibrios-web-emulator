...
const scenarioPath = path.resolve(__dirname, "scenario-calc-keyboard-multiply.json");

if (!fs.existsSync(target)) {
  console.error("CALC not found.");
  console.error("Usage: node tools/autotest-calc-keyboard.js <path-to-calc> [timeout_ms]");
...
(async () => {
  ...
  try {
    ...
    if (!hasExpectedNumber) {
      throw new Error("Keyboard scenario completed without visible number containing '15'.");
    }
    console.log(
      `Autotest OK: CALC keyboard input produced 15 (window ${snapshot.window.width}x${snapshot.window.height}, hash ${snapshot.surface.hash}).`
    );
    process.exit(0);
  } catch (err) {
